...
class FtpIstemci: public Istemci
{
...
public:
	Event Veri_Gonderildi;
	Event Aktarim_Tamamlandi;
	FtpIstemci(char* Domain_IP,int PORT):Istemci(Domain_IP,PORT)
	{
		Veri_Gonderildi = 0;
...
if((Packet.compare(CVP_PASIF)) == 0){ // CVP PASIF
         ...
         #pragma endregion
		 Istemci istemci2(inet_ntoa(this->struct_socketVeri.sin_addr),Port);
		 VekilParams params;
		 params.istemci = &istemci2;
		 params.veri = &dosya;
...
if((Packet.compare(CVP_PASIF)) == 0){ // CVP PASIF
         ...
         #pragma endregion
			 ...
			 Istemci* istemci2;
             istemci2 = new Istemci(inet_ntoa(this->struct_socketVeri.sin_addr),Port);
			 this->PaketTemizle();
